...
export default function NotificationPage() {
  ...
  async function confirmApprove() {
    ...
    try {
      // Call API to approve PO — adapt endpoint to your backend
      // await api.purchaseOrder.approve(logId);
      ...
      // Simulate supplier ACC after 3s (remove in production, use websocket/polling)
      setTimeout(() => {
        setLogs(prev => prev.map(l =>
          l.id === logId ? { ...l, poStatus: "supplier_acc" as POStatus } : l
        ));
      }, 3000);

      // Simulate completed after 6s (remove in production)
      setTimeout(() => {
        setLogs(prev => prev.map(l =>
          l.id === logId ? { ...l, poStatus: "completed" as POStatus } : l
        ));
        setToast({ msg:"Supplier sudah ACC PO! Stok otomatis bertambah.", type:"success" });
      }, 6000);

    } catch (err) {
      setToast({ msg:"Gagal menyetujui PO. Coba lagi.", type:"error" });
    }
  }
