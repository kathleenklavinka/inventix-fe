...
// Badge for incoming PO that will add stock
function POIncomingBadge({ poStatus, jumlah, satuan }: { poStatus: POStatus; jumlah: number; satuan: string }) {
  if (poStatus === "completed") return (
    <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2.5 py-1 rounded-lg" style={{ background:"#CFDECA", color:"#2d6a3f" }}>
      <IconPackageCheck size={10} color="#2d6a3f" /> +{jumlah} {satuan} masuk
    </span>
  );
  if (poStatus === "supplier_acc") return (
    <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2.5 py-1 rounded-lg" style={{ background:"#dbeafe", color:"#1e40af" }}>
      🚚 Dalam pengiriman +{jumlah} {satuan}
    </span>
  );
  if (poStatus === "approved") return (
    <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2.5 py-1 rounded-lg" style={{ background:"#fef3c7", color:"#92400e" }}>
      ⏳ PO disetujui +{jumlah} {satuan}
    </span>
  );
  if (poStatus === "pending") return (
  ...
  const loadStok = async () => {
    try {
      ...
      const mapped: StockItem[] = res.data.map((item: any) => ({
        ...
        satuan: item.satuan,
        // Map PO status from backend if available
  ...
  return (
    <>
      <style>{`
        ...
        .po-incoming-row{background:linear-gradient(90deg,rgba(207,222,202,0.35),rgba(207,222,202,0.10));border-left:3px solid #3B6D11;border-radius:0 8px 8px 0;padding:6px 10px;margin-top:8px;display:flex;align-items:center;gap:8px;font-size:11px;color:#27500A;font-weight:600}

        .stock-incoming-bar{background:repeating-linear-gradient(90deg,#3B6D11 0px,#3B6D11 6px,transparent 6px,transparent 12px);height:100%;border-radius:0 4px 4px 0;opacity:0.40}
      `}</style>
                ...
                <div className="anim-fade-up d300 zoom-card overflow-hidden border"
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {paginated.length === 0 ? (
                          ...
                          const hasPO = item.pendingPO && item.pendingPO.status !== "rejected";
                          return (
                            <tr key={item.id} className="tbl-row"
                              style={{ borderTop:"1px solid rgba(33,33,33,0.05)", background: hasPO ? "rgba(207,222,202,0.12)" : i%2===0 ? "transparent" : "rgba(33,33,33,0.012)" }}>
                              <td className="px-6 py-4 text-[11px] font-bold" style={{ color:"rgba(33,33,33,0.32)" }}>{(page-1)*showCount+i+1}</td>
                              <td className="px-6 py-4 font-semibold text-[#212121] text-xs sm:text-sm">
                                {item.nama}
                                {hasPO && item.pendingPO!.status === "completed" && (
                                  <span className="ml-2 text-[9px] font-bold px-1.5 py-0.5 rounded" style={{ background:"#CFDECA", color:"#27500A" }}>BARU MASUK</span>
                                )}
                              </td>
                              ...
                              <td className="px-6 py-4">
                                <div className="flex items-center gap-2.5">
                                  <div className="w-20 h-1.5 rounded-full overflow-hidden" style={{ background:"rgba(33,33,33,0.09)", position:"relative" }}>
                                    ...
                                    {hasPO && item.pendingPO!.status !== "completed" && (
                                      <div className="stock-incoming-bar" style={{ position:"absolute", top:0, right:0, width:`${Math.min((item.pendingPO!.jumlah/item.max)*100,100-(item.jumlah/item.max)*100)}%` }}/>
                                    )}
                                  </div>
                              ...
                              <td className="px-6 py-4">
                                {hasPO
                                  ? <POIncomingBadge poStatus={item.pendingPO!.status} jumlah={item.pendingPO!.jumlah} satuan={item.satuan}/>
                  ...
                  <div className="sm:hidden divide-y" style={{ borderColor:"rgba(33,33,33,0.06)" }}>
                    {paginated.length === 0 ? (
                      ...
                      const hasPO = item.pendingPO && item.pendingPO.status !== "rejected";
                      return (
                        <div key={item.id} className="tbl-row px-4 py-4" style={{ background: hasPO ? "rgba(207,222,202,0.12)" : "transparent" }}>
              ...
              {viewMode === "grid" && (
                <div className="anim-fade-up d300 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {paginated.length === 0 ? (
                    ...
                    const hasPO = item.pendingPO && item.pendingPO.status !== "rejected";
                    return (
                      <div key={item.id} className="stk-grid-card" style={{ borderColor: hasPO ? "rgba(59,109,17,0.25)" : "rgba(33,33,33,0.09)" }}>
              ...
              <div className="anim-fade-up d300 flex flex-wrap items-center justify-between gap-4">
                <div className="flex items-center gap-4 sm:gap-8 flex-wrap">
                  {[
                    ...
                    { val:`${data.filter(d=>d.pendingPO&&d.pendingPO.status!=="rejected").length}`, label:"PO masuk", success:true },
                  ].map((s,i) => (
                    <div key={i} className="flex items-center gap-4 sm:gap-8">
                      ...
                      <div className="text-center cursor-default">
                        <p className={`font-['Plus_Jakarta_Sans'] font-black text-base sm:text-lg ${(s as any).danger?"text-red-400":(s as any).success?"text-green-400":"text-[#F9F9FA]"}`}>{s.val}</p>
