...
const summaryCards = [
  { label: "Total Item Stok", value: "128", icon: "📦", trend: "naik 4 item minggu ini", trendUp: true, bg: "#D8DFE9", progress: 72 },
  { label: "Penjualan Hari Ini", value: "Rp 4,2jt", icon: "🧾", trend: "naik 12% vs kemarin", trendUp: true, bg: "#CFDECA", progress: 84 },
  { label: "Jumlah Supplier", value: "14", icon: "🏭", trend: "sama seperti kemarin", trendUp: null, bg: "#EFF0A3", progress: 50 },
  { label: "Jumlah User", value: "6", icon: "👤", trend: "naik 1 user baru", trendUp: true, bg: "#D8DFE9", progress: 60 },
];
...
const recentActivity = [
  { action: "Stok Gula Pasir diperbarui", time: "5 menit lalu", icon: "📝", color: "#EFF0A3" },
  { action: "Penjualan baru: Kopi Arabika x3", time: "23 menit lalu", icon: "🧾", color: "#CFDECA" },
  { action: "Supplier Mitra Jaya ditambahkan", time: "1 jam lalu", icon: "🏭", color: "#D8DFE9" },
  { action: "User Budi Santoso bergabung", time: "3 jam lalu", icon: "👤", color: "#D8DFE9" },
  { action: "Stok Minyak Goreng habis", time: "5 jam lalu", icon: "⚠️", color: "#fee2e2" },
];
...
function Inner({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={`max-w-6xl mx-auto px-4 sm:px-8 ${className}`}>{children}</div>
  );
}
  ...
  return (
    <>
      <style>{`
        ...
        /* ── ZOOM CARD hover ── */
        .zoom-card {
          transition: transform 0.30s cubic-bezier(.22,1,.36,1),
                      box-shadow 0.30s cubic-bezier(.22,1,.36,1);
          will-change: transform;
          cursor: pointer;
        }
        .zoom-card:hover {
          transform: scale(1.045);
          box-shadow: 0 20px 48px rgba(33,33,33,0.13);
        }
        ...
        .act-row {
          border-radius: 12px;
          transition: background 0.18s, transform 0.25s cubic-bezier(.22,1,.36,1), box-shadow 0.22s;
        }
        .act-row:hover {
          background: rgba(255,255,255,0.70);
          transform: translateX(6px) scale(1.015);
          box-shadow: 0 4px 14px rgba(33,33,33,0.07);
        }
        .act-icon {
          transition: transform 0.28s cubic-bezier(.22,1,.36,1);
        }
        .act-row:hover .act-icon {
          transform: scale(1.18) rotate(6deg);
        }

        /* table row */
        .tbl-row {
          transition: background 0.18s, transform 0.22s cubic-bezier(.22,1,.36,1);
        }
        .tbl-row:hover {
          background: rgba(255,255,255,0.65) !important;
          transform: scale(1.012) translateX(3px);
        }
        ...
        .bar-col {
          transition: height 0.55s cubic-bezier(.22,1,.36,1), background 0.2s;
        }
        .bar-wrap {
          transition: transform 0.22s cubic-bezier(.22,1,.36,1);
        }
        ...
        /* welcome card shimmer border on hover */
        .welcome-card {
          transition: box-shadow 0.30s cubic-bezier(.22,1,.36,1), transform 0.30s cubic-bezier(.22,1,.36,1);
        }
        .welcome-card:hover {
          transform: translateY(-3px);
          box-shadow: 0 16px 48px rgba(33,33,33,0.09);
        }
      `}</style>
      ...
      >
        ...
        <main className="w-full">
          ...
          {/* SECTION 1 - HERO */}
          <section
            className="w-full relative overflow-hidden pt-16 pb-12 sm:pt-20 sm:pb-14"
          ...
          >
            ...
            <Inner>
              {/* Welcome card */}
              <div
                className="anim-fade-up welcome-card relative z-10 border px-6 sm:px-10 py-7 sm:py-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5"
                style={{
                  background: "rgba(255,255,255,0.78)",
                  backdropFilter: "blur(18px)",
                  borderColor: "rgba(255,255,255,0.9)",
                  borderRadius: "20px",
                  boxShadow: "0 4px 24px rgba(33,33,33,0.07), inset 0 1px 0 rgba(255,255,255,0.9)",
                }}
              >

                <div className="absolute left-0 top-6 bottom-6 w-1 rounded-full" style={{ background: "linear-gradient(180deg, #D8DFE9, #CFDECA, #EFF0A3)", borderRadius: "0 4px 4px 0" }} />
                ...
                <div className="pl-3">
                  ...
                  <div className="flex items-center gap-2.5 sm:gap-3 flex-wrap">
                    ...
                    <span
                      className="font-bold text-[10px] sm:text-xs px-3 py-1"
                ...
                <div className="flex flex-row sm:flex-col items-center sm:items-end gap-5 sm:gap-2.5">
                  <div
                    className="font-['Plus_Jakarta_Sans'] font-black tabular-nums tracking-tight text-[#212121]"
              ...
              {adaStokHabis && (
                <div
                  className="anim-fade-up d200 relative z-10 mt-3.5 flex items-center gap-3 px-4 sm:px-5 py-3.5 border"
                ...
                >
                  ...
                  <p className="text-xs sm:text-sm font-medium text-red-600">
                    Terdapat barang dengan stok <span className="font-bold">0</span>. Segera lakukan restok.
                  </p>
                  <Link
                    href="/stock"
                    className="ml-auto text-[10px] sm:text-xs font-bold px-3 py-1.5 border transition-all flex-shrink-0 hover:bg-red-50 active:scale-95"
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
                {summaryCards.map((card, i) => (
                  ...
                  >
                    ...
                    <div className="absolute bottom-3 right-3 w-6 h-6 rounded-full bg-white/20 pointer-events-none" />
                    <div className="flex items-start justify-between relative z-10">
                      <span className="text-xl sm:text-2xl">{card.icon}</span>
                      <span className={`text-[9px] sm:text-[10px] font-bold px-2 py-0.5 rounded-full ${card.trendUp === true ? "bg-emerald-100 text-emerald-700" : card.trendUp === false ? "bg-red-100 text-red-600" : "bg-white/45 text-[#212121]/45"}`}>
                        {card.trendUp === true ? "↑ naik" : card.trendUp === false ? "↓ turun" : "tetap"}
                      </span>
                    </div>
          ...
          <section className="w-full relative overflow-hidden py-10 sm:py-12" style={{ background: "#edf5f0" }}>
            ...
            <Inner className="relative z-10">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-5 gap-4">
                ...
                {/* Bar Chart */}
                <div
                  className="anim-fade-up d300 sm:col-span-3 zoom-card p-5 sm:p-7 border"
                ...
                >
                  <div className="flex items-start justify-between mb-6">
                    ...
                    <span className="text-[10px] sm:text-xs font-bold text-emerald-600 bg-emerald-50 border border-emerald-100 px-2.5 py-1 rounded-xl">+18%</span>
                  </div>
                  <div className="flex items-end gap-2 sm:gap-3.5" style={{ height: "120px" }}>
                    {salesChart.map((d, i) => {
                      ...
                      return (
                        <div
                          key={i}
                          className="flex-1 flex flex-col items-center gap-2 cursor-pointer group bar-wrap"
                        ...
                        >
                          ...
                          <div className="w-full flex items-end rounded-t-xl overflow-hidden" style={{ height: "86px" }}>
                            <div
                              className={`w-full rounded-t-xl bar-col`}
                              style={{
                ...
                {/* Activity Feed */}
                <div
                  className="anim-fade-up d400 sm:col-span-2 zoom-card p-5 sm:p-6 border"
                ...
                >
                  ...
                  <div className="space-y-1">
                    {recentActivity.map((item, i) => (
                      <div
                        key={i}
                        className="anim-fade-up act-row flex items-center gap-3 px-2.5 py-2.5 cursor-default"
                        style={{ animationDelay: `${0.4 + i * 0.07}s` }}
                      >
                        <div
                          className="act-icon w-7 h-7 sm:w-8 sm:h-8 rounded-xl flex items-center justify-center flex-shrink-0 text-xs"
          ...
          >
            ...
            <Inner className="relative z-10">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-5 gap-4">
                ...
                {/* Top Barang */}
                <div
                  className="anim-fade-up d350 sm:col-span-2 zoom-card p-5 sm:p-6 border"
                      ...
                      <div key={i} className="space-y-1.5">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2.5">
                            <span
                              className="font-['Plus_Jakarta_Sans'] font-black text-xs w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0"
                ...
                {/* Tabel Stok */}
                <div
                  className="anim-fade-up d450 sm:col-span-3 zoom-card overflow-hidden border"
                ...
                >
                  <div className="px-5 sm:px-7 py-4 sm:py-4.5 flex items-center justify-between" style={{ borderBottom: "1px solid rgba(33,33,33,0.07)" }}>
                    <h2 className="font-['Plus_Jakarta_Sans'] font-bold text-sm sm:text-[15px] text-[#212121]">Ringkasan Stok</h2>
                    <Link href="/stock" className="text-[10px] sm:text-xs font-bold transition-colors hover:text-[#212121] active:scale-95" style={{ color: "rgba(33,33,33,0.32)" }}>
                      Lihat semua
                    </Link>
                  </div>
                  ...
                  <div className="sm:hidden divide-y" style={{ borderColor: "rgba(33,33,33,0.06)" }}>
                    {stokTerbaru.map((item, i) => (
                      <div key={i} className="tbl-row px-5 py-3.5 flex items-center justify-between gap-3">
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-1.5">
                            <div className="flex-1 h-1 rounded-full overflow-hidden" style={{ background: "rgba(33,33,33,0.08)" }}>
                              <div
                                className={`h-full rounded-full progress-bar ${item.jumlah === 0 ? "bg-red-400" : "bg-[#CFDECA]"}`}
                        ...
                        {item.jumlah === 0
                          ? <span className="bg-red-100 text-red-600 text-[9px] font-bold px-2 py-0.5 rounded-lg flex-shrink-0">Habis</span>
                          : <span className="bg-[#CFDECA] text-[#212121] text-[9px] font-bold px-2 py-0.5 rounded-lg flex-shrink-0">Tersedia</span>
                        }
                      </div>
                    ))}
                  </div>
                      ...
                      <tbody>
                        {stokTerbaru.map((item, i) => (
                          <tr key={i} className="tbl-row" style={{ borderTop: "1px solid rgba(33,33,33,0.05)" }}>
                            ...
                            <td className="px-6 py-4">
                              <div className="flex items-center gap-2.5">
                                <div className="w-20 h-1.5 rounded-full overflow-hidden" style={{ background: "rgba(33,33,33,0.09)" }}>
                                  <div
                                    className={`h-full rounded-full progress-bar ${item.jumlah === 0 ? "bg-red-400" : item.jumlah / item.max < 0.3 ? "bg-[#EFF0A3]" : "bg-[#CFDECA]"}`}
                            ...
                            <td className="px-6 py-4">
                              {item.jumlah === 0
                                ? <span className="bg-red-100 text-red-600 text-[10px] font-bold px-2.5 py-1 rounded-lg">Habis</span>
                                : item.jumlah / item.max < 0.3
                                ? <span className="bg-[#EFF0A3] text-[#212121] text-[10px] font-bold px-2.5 py-1 rounded-lg">Menipis</span>
                                : <span className="bg-[#CFDECA] text-[#212121] text-[10px] font-bold px-2.5 py-1 rounded-lg">Tersedia</span>
                              }
                            </td>
                          </tr>
                        ))}
          ...
          <section className="w-full relative overflow-hidden py-5 sm:py-6" style={{ background: "#212121" }}>
            <div className="absolute -top-12 right-10 w-44 h-44 rounded-full opacity-6 blob pointer-events-none" style={{ background: "#EFF0A3", filter: "blur(44px)" }} />
            <Inner>
              <div className="anim-fade-up d550 flex flex-wrap items-center justify-between gap-4">
                <div className="flex items-center gap-4 sm:gap-8 flex-wrap">
                  {[
                    ...
                    { val: "2",     label: "Stok habis",         danger: true  },
                  ].map((stat, i) => (
                    <div key={i} className="flex items-center gap-4 sm:gap-8">
                      ...
                      <div className="stat-item text-center cursor-default">
                        <p
                          className={`stat-num font-['Plus_Jakarta_Sans'] font-black text-base sm:text-lg ${stat.danger ? "text-red-400" : ""}`}
