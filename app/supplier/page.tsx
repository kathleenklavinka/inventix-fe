...
const PO_STATUS_CFG: Record<POStatus, { label: string; bg: string; text: string; border: string; iconType: string }> = {
...
function AccModal({ po, supplierNama, onConfirm, onCancel }: {
  po: PurchaseOrder; supplierNama: string; onConfirm: () => void; onCancel: () => void;
}) {
  return (
    <div style={{ position:"fixed", inset:0, zIndex:60, display:"flex", alignItems:"center", justifyContent:"center", padding:16, background:"rgba(2,44,34,0.50)", backdropFilter:"blur(6px)" }} onClick={onCancel}>
      <div style={{ background:"#FFFFFF", borderRadius:20, padding:"28px 28px 24px", maxWidth:420, width:"100%", boxShadow:"0 24px 64px rgba(2,44,34,0.25)", border:"1.5px solid #A8DEBC" }} onClick={e => e.stopPropagation()}>
        ...
        <h2 style={{ fontSize:16, fontWeight:800, color:"#022c22", margin:"0 0 6px", fontFamily:"var(--font-plus-jakarta),sans-serif" }}>ACC Purchase Order?</h2>
        <p style={{ fontSize:12, color:"#4A6B52", margin:"0 0 4px" }}>Nomor PO: <strong style={{ color:"#022c22" }}>{po.nomorPO}</strong></p>
        <p style={{ fontSize:12, color:"#4A6B52", margin:"0 0 4px" }}>Barang: <strong style={{ color:"#022c22" }}>{po.namaBarang}</strong> · {po.jumlah} {po.satuan}</p>
        <p style={{ fontSize:12, color:"#4A6B52", margin:"0 0 16px" }}>Nilai: <strong style={{ color:"#1A6647" }}>{formatRupiah(po.nilaiTotal)}</strong></p>
        ...
        <div style={{ display:"flex", gap:10 }}>
          <button onClick={onCancel} style={{ flex:1, padding:"10px", borderRadius:10, border:"1.5px solid #A8DEBC", background:"#FFFFFF", color:"#4A6B52", fontSize:12, fontWeight:700, cursor:"pointer", fontFamily:"var(--font-plus-jakarta),sans-serif" }}>Batal</button>
          <button onClick={onConfirm} style={{ flex:2, padding:"10px", borderRadius:10, border:"none", background:"linear-gradient(135deg,#3A8F46,#2D6B37)", color:"#fff", fontSize:12, fontWeight:700, cursor:"pointer", fontFamily:"var(--font-plus-jakarta),sans-serif", display:"flex", alignItems:"center", justifyContent:"center", gap:6 }}>
...
function RejectModal({ po, onConfirm, onCancel }: {
  po: PurchaseOrder; onConfirm: (alasan: string) => void; onCancel: () => void;
}) {
  const [alasan, setAlasan] = useState("");
  return (
    <div style={{ position:"fixed", inset:0, zIndex:60, display:"flex", alignItems:"center", justifyContent:"center", padding:16, background:"rgba(2,44,34,0.50)", backdropFilter:"blur(6px)" }} onClick={onCancel}>
      <div style={{ background:"#FFFFFF", borderRadius:20, padding:"28px 28px 24px", maxWidth:420, width:"100%", boxShadow:"0 24px 64px rgba(2,44,34,0.25)", border:"1.5px solid #E8A8A8" }} onClick={e => e.stopPropagation()}>
        ...
        <h2 style={{ fontSize:16, fontWeight:800, color:"#022c22", margin:"0 0 6px", fontFamily:"var(--font-plus-jakarta),sans-serif" }}>Tolak Purchase Order?</h2>
        <p style={{ fontSize:12, color:"#4A6B52", margin:"0 0 14px" }}>Nomor PO: <strong style={{ color:"#022c22" }}>{po.nomorPO}</strong> · {po.namaBarang}</p>
        <div style={{ marginBottom:16 }}>
          <label style={{ fontSize:11, fontWeight:700, color:"#8A2020", display:"block", marginBottom:6 }}>Alasan penolakan (opsional)</label>
          <textarea value={alasan} onChange={e => setAlasan(e.target.value)} placeholder="Contoh: Stok sudah tersedia, harga tidak sesuai, dll."
        ...
        <div style={{ display:"flex", gap:10 }}>
          <button onClick={onCancel} style={{ flex:1, padding:"10px", borderRadius:10, border:"1.5px solid #A8DEBC", background:"#FFFFFF", color:"#4A6B52", fontSize:12, fontWeight:700, cursor:"pointer", fontFamily:"var(--font-plus-jakarta),sans-serif" }}>Batal</button>
          <button onClick={() => onConfirm(alasan)} style={{ flex:2, padding:"10px", borderRadius:10, border:"none", background:"linear-gradient(135deg,#C85040,#A83030)", color:"#fff", fontSize:12, fontWeight:700, cursor:"pointer", fontFamily:"var(--font-plus-jakarta),sans-serif", display:"flex", alignItems:"center", justifyContent:"center", gap:6 }}>
...
function PODrawer({ supplier, userRole, onClose, onAcc, onReject }: {
  supplier: SupplierItem; userRole: string; onClose: () => void;
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:50, display:"flex", justifyContent:"flex-end", background:"rgba(2,44,34,0.40)", backdropFilter:"blur(5px)" }} onClick={onClose}>
      <div style={{ width:"100%", maxWidth:520, height:"100%", overflowY:"auto", background:"#F0FDF4", borderLeft:"1.5px solid #A8DEBC", padding:"24px 24px 40px", display:"flex", flexDirection:"column", gap:16, animation:"slideInDrawer .30s cubic-bezier(.22,1,.36,1) both" }} onClick={e => e.stopPropagation()}>
        <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:4 }}>
          ...
          <button onClick={onClose} style={{ width:32, height:32, borderRadius:10, border:"1.5px solid #A8DEBC", background:"#FFFFFF", display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer" }}>
        ...
        <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
          {[
            ...
            <span key={i} style={{ fontSize:10, fontWeight:700, padding:"4px 10px", borderRadius:20, background:p.bg, color:p.text, border:`1.5px solid ${p.border}` }}>{p.label}</span>
          ))}
        </div>
        {activePOs.length === 0 ? (
          ...
          <div>
            <p style={{ fontSize:10, fontWeight:700, letterSpacing:"0.09em", color:"rgba(6,78,59,0.50)", textTransform:"uppercase", marginBottom:10 }}>PO Aktif</p>
            <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
              {activePOs.map(po => {
                const needsAcc   = po.status === "approved";
                const inDelivery = po.status === "supplier_acc";
                return (
                  <div key={po.id} style={{ background: needsAcc?"#FFFAF0":inDelivery?"#F4FFF8":"#FFFFFF", border:`1.5px solid ${needsAcc?"#F5C878":inDelivery?"#A8DEBC":"#D0E8D4"}`, borderLeft:`4px solid ${needsAcc?"#E8A850":inDelivery?"#3A8F46":"#81C3DE"}`, borderRadius:"0 14px 14px 0", padding:"14px 16px" }}>
                    <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:8, marginBottom:8 }}>
                      <div>
                        <p style={{ fontSize:12, fontWeight:800, color:"#022c22", margin:0 }}>{po.nomorPO}</p>
                        <p style={{ fontSize:11, color:"rgba(6,78,59,0.65)", margin:"2px 0 0" }}>🛒 {po.namaBarang} · {po.jumlah} {po.satuan}</p>
                      </div>
                      <span style={{ fontSize:12, fontWeight:800, color:needsAcc?"#8A4A00":"#1A6647", background:needsAcc?"#FEF0D6":"#D6F5E8", padding:"3px 10px", borderRadius:20, whiteSpace:"nowrap" }}>{formatRupiah(po.nilaiTotal)}</span>
                    </div>
                    <div style={{ marginBottom:10 }}><POStatusBadge status={po.status}/></div>
                    {needsAcc && (
                      <div style={{ display:"flex", gap:8 }}>
                        <button onClick={() => onAcc(supplier.id, po.id)} style={{ display:"inline-flex", alignItems:"center", gap:6, padding:"9px 14px", borderRadius:10, border:"none", background:"linear-gradient(135deg,#3A8F46,#2D6B37)", color:"#fff", fontSize:11, fontWeight:800, cursor:"pointer", fontFamily:"var(--font-plus-jakarta),sans-serif", flex:2 }}>
                          <IconTruck size={13} color="#fff"/> ACC & Siapkan Pengiriman
                        </button>
                        <button onClick={() => onReject(supplier.id, po.id)} style={{ display:"inline-flex", alignItems:"center", gap:6, padding:"9px 14px", borderRadius:10, border:"1.5px solid #E8A8A8", background:"#FFF0F0", color:"#C83030", fontSize:11, fontWeight:800, cursor:"pointer", fontFamily:"var(--font-plus-jakarta),sans-serif", flex:1 }}>
                          <IconX size={11} color="#C83030"/> Tolak
                        </button>
                      </div>
                    )}
                    {inDelivery && (
                      <div style={{ display:"flex", alignItems:"center", gap:6, padding:"8px 10px", borderRadius:10, background:"rgba(78,168,90,0.10)", border:"1px solid #A8DEBC" }}>
                        ...
                        <p style={{ fontSize:11, color:"#1A6647", fontWeight:600, margin:0 }}>Sedang dalam pengiriman — stok otomatis bertambah saat tiba</p>
                      </div>
                    )}
                    {po.catatan && (
                      <div style={{ display:"flex", gap:3, alignItems:"flex-start", margin:"8px 0 0" }}>
                        <span style={{ flexShrink:0, marginTop:"2px", display:"flex" }}><IconEdit size={10} color="rgba(6,78,59,0.55)"/></span>
                        <p style={{ fontSize:10, color:"rgba(6,78,59,0.55)", fontStyle:"italic", margin:0 }}>{po.catatan}</p>
                      </div>
                    )}
                  </div>
                );
        ...
        {historyPOs.length > 0 && (
          <div>
            <p style={{ fontSize:10, fontWeight:700, letterSpacing:"0.09em", color:"rgba(6,78,59,0.50)", textTransform:"uppercase", marginBottom:10, marginTop:8 }}>Riwayat PO</p>
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              {historyPOs.map(po => (
                <div key={po.id} style={{ background:"#FFFFFF", border:"1.5px solid rgba(6,78,59,0.10)", borderRadius:12, padding:"12px 14px", opacity:po.status==="rejected"?0.7:1 }}>
...
function Toast({ msg, type, onClose }: { msg: string; type: "success"|"error"|"info"; onClose: () => void }) {
  ...
  return (
    <div style={{ position:"fixed", bottom:24, right:24, zIndex:70, display:"flex", alignItems:"center", gap:10, padding:"12px 18px", borderRadius:14, background:c.bg, border:`1.5px solid ${c.border}`, boxShadow:"0 8px 32px rgba(0,0,0,0.15)", minWidth:280, maxWidth:400, animation:"slideInRight .3s cubic-bezier(.22,1,.36,1)" }}>
      ...
      <button onClick={onClose} style={{ background:"none", border:"none", cursor:"pointer" }}><IconX size={16} color={c.text}/></button>
    </div>
  );
}
...
export default function SupplierPage() {
      ...
      try {
        ...
        if (poRes?.data) {
          poRes.data.forEach((po: any) => {
            ...
            else if (rawStatus === "supplier_acc" || rawStatus === "dikonfirmasi" || rawStatus === "supplieracc") mappedStatus = "supplier_acc";
  ...
  async function confirmAcc() {
    ...
    setAccModal(null);
    try {
      setData(prev => prev.map(sup => sup.id !== supplierId ? sup : {
        ...sup,
        purchaseOrders: sup.purchaseOrders.map(p => p.id === po.id ? { ...p, status: "supplier_acc" as POStatus } : p),
        pendingPOCount: sup.pendingPOCount - 1,
      }));
      if (activeDrawer?.id === supplierId) {
        setActiveDrawer(prev => prev ? ({
          ...prev,
          purchaseOrders: prev.purchaseOrders.map(p => p.id === po.id ? { ...p, status: "supplier_acc" as POStatus } : p),
      ...
      setTimeout(() => {
        setData(prev => prev.map(sup => sup.id !== supplierId ? sup : {
          ...sup,
          purchaseOrders: sup.purchaseOrders.map(p => p.id === po.id ? { ...p, status: "completed" as POStatus } : p),
        }));
        setToast({ msg:`PO ${po.nomorPO} selesai — stok ${po.namaBarang} +${po.jumlah} ${po.satuan}!`, type:"success" });
      }, 5000);
    } catch (err: any) {
  ...
  async function confirmReject(alasan: string) {
    ...
    try {
      setData(prev => prev.map(sup => sup.id !== supplierId ? sup : {
        ...sup,
        purchaseOrders: sup.purchaseOrders.map(p => p.id === po.id ? { ...p, status: "rejected" as POStatus, catatan: alasan || p.catatan } : p),
        pendingPOCount: Math.max(0, sup.pendingPOCount - 1),
      }));
      if (activeDrawer?.id === supplierId) {
        setActiveDrawer(prev => prev ? ({
          ...prev,
          purchaseOrders: prev.purchaseOrders.map(p => p.id === po.id ? { ...p, status: "rejected" as POStatus } : p),
        }) : prev);
      }
      setToast({ msg:`PO ${po.nomorPO} ditolak.`, type:"error" });
    } catch (err: any) {
  ...
  async function handleDelete(item: SupplierItem) {
    try {
      await api.supplier.delete(parseInt(item.id, 10));
  ...
  // FIX pagination: pisahkan dulu array, baru render
  const pageItems = Array.from({ length: totalPages }, (_, i) => i + 1)
    .filter(p => p === 1 || p === totalPages || Math.abs(p - page) <= 1);

  const pageItemsWithEllipsis = pageItems.reduce<(number | "…")[]>((acc, p, idx) => {
    const prev = pageItems[idx - 1];
    if (idx > 0 && typeof prev === "number" && p - prev > 1) acc.push("…");
    acc.push(p);
    return acc;
  }, []);
      ...
      <div className={`min-h-screen text-[#064e3b] relative overflow-x-hidden transition-opacity duration-500 ${mounted?"opacity-100":"opacity-0"}`}
        ...
        <main>
          <section className="w-full relative overflow-hidden pt-16 pb-10 sm:pt-20 sm:pb-12"
            ...
            <div className="absolute -top-24 -right-24 w-80 h-80 rounded-full blob-g pointer-events-none" style={{ background:"#6ee7b7", filter:"blur(80px)", opacity:0.28 }}/>
            <div className="absolute -bottom-12 -left-12 w-64 h-64 rounded-full blob-g2 pointer-events-none" style={{ background:"#a7f3d0", filter:"blur(60px)", opacity:0.22 }}/>
            <Inner className="relative z-10">
              ...
              <div className="su-fade-up d2 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
                <div>
                  ...
                  <h1 className="font-['Plus_Jakarta_Sans'] font-black text-2xl sm:text-[2.1rem] leading-none" style={{ color:"#022c22" }}>Data Supplier</h1>
                  <p className="text-[11px] mt-1.5 font-medium" style={{ color:"rgba(6,78,59,0.38)" }}>{tanggal}</p>
                </div>
                {isAdmin && (
                  <div className="flex items-center gap-3 flex-wrap">
                    <Link href="/supplier/po"><button className="btn-po"><IconFileText size={13} color="#064e3b"/> Buat PO</button></Link>
                    <Link href="/supplier/tambah"><button className="btn-green"><IconPlus size={14} color="#d1fae5"/> Tambah Supplier</button></Link>
                  </div>
                )}
              </div>
              ...
              {totalPONeedAcc > 0 && (
                <div className="su-fade-up d5 mt-4 flex items-center gap-3 px-4 sm:px-5 py-3.5 border"
                  ...
                  <span style={{ marginLeft:"auto", fontSize:11, color:"#92400e", fontWeight:700 }}>Klik nama supplier untuk melihat →</span>
                </div>
              )}
            </Inner>
          </section>
          ...
          <section className="w-full py-10 sm:py-12" style={{ background:"#ffffff" }}>
            <Inner>
                      ...
                      <thead>
                        <tr style={{ background:"rgba(6,78,59,0.03)", borderBottom:"1px solid rgba(6,78,59,0.07)" }}>
                          {[
                            ...
                            <th key={i} className={`px-5 py-3.5 text-left text-[10px] font-bold tracking-[0.08em] uppercase ${h.col?"sort-th":""}`}
                          ...
                          return (
                            <tr key={item.id} className="tbl-row"
                              ...
                              <td className="px-5 py-4">
                                <button onClick={() => setActiveDrawer(item)} className="flex items-center gap-3 group"
                      ...
                      return (
                        <div key={item.id} className="tbl-row px-4 py-4" style={{ background:hasPOAcc?"rgba(254,243,199,0.15)":"transparent" }}>
                          ...
                          <div className="space-y-1 mb-3 pl-11">
                            <p className="text-[11px] flex items-start gap-1.5" style={{ color:"rgba(6,78,59,0.55)" }}><IconMapPin size={11} color="rgba(6,78,59,0.40)"/> {item.alamat}</p>
                            <p className="text-[11px] flex items-center gap-1.5" style={{ color:"rgba(6,78,59,0.55)" }}><IconPhone size={11} color="rgba(6,78,59,0.40)"/> {item.telepon}</p>
                          </div>
                          {item.purchaseOrders.filter(p => p.status !== "completed" && p.status !== "rejected").length > 0 && (
                      ...
                      <div key={item.id} className="sup-grid-card" style={{ borderColor:hasPOAcc?"rgba(245,200,120,0.50)":"rgba(6,78,59,0.10)", background:hasPOAcc?"rgba(254,243,199,0.12)":"rgba(255,255,255,0.75)" }}>
                        ...
                        <div className="space-y-2 py-3 border-t border-b mb-3" style={{ borderColor:"rgba(6,78,59,0.08)" }}>
                          <p className="text-[11px] flex items-start gap-2" style={{ color:"rgba(6,78,59,0.55)" }}>
                            <span className="flex-shrink-0 mt-0.5"><IconMapPin size={11} color="rgba(6,78,59,0.42)"/></span>{item.alamat}
                          </p>
                          <p className="text-[11px] flex items-center gap-2" style={{ color:"rgba(6,78,59,0.55)" }}><IconPhone size={11} color="rgba(6,78,59,0.42)"/> {item.telepon}</p>
                          {item.purchaseOrders.filter(p => p.status !== "completed" && p.status !== "rejected").length > 0 && (
                            <button onClick={() => setActiveDrawer(item)} className={`po-badge ${hasPOAcc?"po-badge-pulse":""}`}
              ...
              {/* Pagination — dipisah jadi variabel dulu, type-safe */}
