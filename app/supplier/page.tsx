...
export default function SupplierPage() {
  ...
  const statItems = [
    ...
    { label:"Nonaktif",       val:data.length - aktifCount,  valColor:"#b45309", iconBg:"#fef3c7", icon:<IconAlertTri2/> },
              ...
              <div className="su-fade-up d2 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
                <div>
                  ...
                  <h1 className="font-['Plus_Jakarta_Sans'] font-black text-2xl sm:text-[2.1rem] leading-none"
                    style={{ color:"#022c22" }}>Data Supplier</h1>
                ...
                {isAdmin && (
                  <div className="flex items-center gap-3">
                    ...
                    <Link href="/supplier/po">
                      <button className="btn-po">
                        <IconFileText size={13} color="#064e3b" /> Buat PO
                      </button>
                    </Link>
                            ...
                            {isAdmin && (
                              <td className="px-5 py-4">
                                <div className="flex items-center gap-2">
                                  {/* Buat PO per supplier */}
                                  <Link href={`/supplier/po?supplier=${item.id}&nama=${encodeURIComponent(item.nama)}`}>
                                    <button className="btn-po" title="Buat PO untuk supplier ini">
                        ...
                        {isAdmin && (
                          <div className="flex gap-2 pl-11">
                            <Link href={`/supplier/po?supplier=${item.id}&nama=${encodeURIComponent(item.nama)}`} className="flex-1">
                              <button className="btn-po w-full justify-center"><IconFileText size={12}/> Buat PO</button>
                            </Link>
