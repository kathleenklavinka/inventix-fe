...
const IconUserX        = ({ size = 18, color = "currentColor" }) => <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="8.5" cy="7" r="4"/><line x1="18" y1="8" x2="23" y2="13"/><line x1="23" y1="8" x2="18" y2="13"/></svg>;
...
const IconAlertTri     = ({ size = 16, color = "currentColor" }) => <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>;
const IconCalendar     = ({ size = 11, color = "currentColor" }) => <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>;
const IconGrid         = ({ size = 16, color = "currentColor" }) => <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>;
const IconList         = ({ size = 16, color = "currentColor" }) => <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/><line x1="3" y1="12" x2="3.01" y2="12"/><line x1="3" y1="18" x2="3.01" y2="18"/></svg>;
...
function RoleBadge({ role }: { role: string }) {
  const styles: Record<string, { bg: string; color: string; icon: React.ReactNode; label: string }> = {
    owner:    { bg: "#f5e6cc", color: "#92400e", icon: <IconCrown size={9} color="#92400e" />, label: "Owner" },
    admin:    { bg: "#e8dfc8", color: "#7a5c2e", icon: <IconShield size={9} color="#7a5c2e" />, label: "Admin" },
    user:     { bg: "#D8DFE9", color: "#2a3a52", icon: <IconUser size={9} color="#2a3a52" />, label: "User" },
    supplier: { bg: "#e0d5f5", color: "#5b21b6", icon: <IconUser size={9} color="#5b21b6" />, label: "Supplier" },
  };
  ...
  return (
    <>
      <style>{`
        ...
        .btn-primary{background:#2a1f08;color:#ffffff;border:none;border-radius:10px;font-family:'Plus Jakarta Sans',sans-serif;font-weight:700;padding:9px 18px;font-size:13px;cursor:pointer;transition:transform .2s cubic-bezier(.22,1,.36,1),box-shadow .2s,background .15s;display:inline-flex;align-items:center;gap:6px;}
        .btn-primary:hover{transform:translateY(-2px) scale(1.03);box-shadow:0 8px 24px rgba(42,31,8,.22);background:#3d2e0e}
        .btn-primary:active{transform:scale(0.97)}

        .btn-ghost{background:rgba(33,33,33,0.06);color:#212121;border:none;border-radius:8px;font-family:'Plus Jakarta Sans',sans-serif;font-weight:600;padding:6px 12px;font-size:12px;cursor:pointer;transition:background .15s,transform .18s;display:inline-flex;align-items:center;gap:5px;}
        .btn-ghost:hover{background:rgba(33,33,33,0.11);transform:scale(1.02)}

        .btn-danger{background:rgba(220,38,38,0.07);color:#dc2626;border:none;border-radius:8px;font-family:'Plus Jakarta Sans',sans-serif;font-weight:600;padding:6px 12px;font-size:12px;cursor:pointer;transition:background .15s,transform .18s;display:inline-flex;align-items:center;gap:5px;}
        .btn-danger:hover{background:rgba(220,38,38,0.13);transform:scale(1.02)}

        .btn-danger-locked{background:rgba(33,33,33,0.05);color:rgba(33,33,33,0.28);border:none;border-radius:8px;font-family:'Plus Jakarta Sans',sans-serif;font-weight:600;padding:6px 12px;font-size:12px;cursor:not-allowed;display:inline-flex;align-items:center;gap:5px;position:relative;}
        .btn-danger-locked .lock-tip{display:none;position:absolute;bottom:calc(100% + 6px);left:50%;transform:translateX(-50%);background:#1a1a1a;color:#fff;font-size:10px;font-weight:500;white-space:nowrap;padding:5px 8px;border-radius:7px;pointer-events:none;z-index:20;}
        .btn-danger-locked:hover .lock-tip{display:block;}
        .btn-danger-locked .lock-tip::after{content:'';position:absolute;top:100%;left:50%;transform:translateX(-50%);border:4px solid transparent;border-top-color:#1a1a1a;}

        .search-input{border:1.5px solid rgba(33,33,33,0.12);border-radius:10px;padding:8px 12px 8px 34px;font-size:13px;background:#ffffff;color:#212121;outline:none;width:100%;font-family:'Inter',sans-serif;transition:border-color .2s,box-shadow .2s;}
        .search-input:focus{border-color:rgba(33,33,33,0.30);box-shadow:0 0 0 3px rgba(33,33,33,0.05)}

        .user-card{background:#ffffff;border:1px solid rgba(33,33,33,0.08);border-radius:16px;padding:1.25rem 1rem 1rem;display:flex;flex-direction:column;align-items:center;text-align:center;position:relative;transition:border-color .2s,box-shadow .25s,transform .25s cubic-bezier(.22,1,.36,1);}
        .user-card:hover{border-color:rgba(33,33,33,0.18);box-shadow:0 12px 32px rgba(33,33,33,0.07);transform:translateY(-2px)}

        .list-row{background:#ffffff;border:1px solid rgba(33,33,33,0.08);border-radius:12px;padding:12px 16px;display:flex;align-items:center;gap:14px;transition:border-color .18s,box-shadow .2s;}
        .list-row:hover{border-color:rgba(33,33,33,0.18);box-shadow:0 4px 16px rgba(33,33,33,0.05)}

        .stat-card{background:#ffffff;border:1px solid rgba(33,33,33,0.08);border-radius:14px;padding:14px 16px;display:flex;align-items:center;gap:12px;transition:box-shadow .2s,transform .2s cubic-bezier(.22,1,.36,1);cursor:default;}
        .stat-card:hover{box-shadow:0 8px 24px rgba(33,33,33,0.07);transform:translateY(-1px)}

        .page-btn{width:30px;height:30px;border-radius:7px;border:1px solid transparent;font-size:12px;font-family:'Plus Jakarta Sans',sans-serif;font-weight:600;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;transition:all .15s;}
        ...
        .role-banner{display:flex;align-items:center;gap:8px;padding:10px 14px;border-radius:11px;font-size:11px;font-weight:600;margin-bottom:16px;}
      `}</style>
            ...
            <Inner>
              ...
              {blockedToast && (
                <div className="err-bar flex items-center gap-2.5 px-4 py-3 rounded-xl mb-4"
                  ...
                  <IconAlertTri size={15} color="#dc2626" />
                  <p className="text-sm font-semibold text-red-600">{blockedToast}</p>
                </div>
              )}
              ...
              <div className="anim-fade-up d250 flex flex-col sm:flex-row sm:items-center gap-3 mb-4">
                <div className="relative flex-1 max-w-xs">
                  ...
                  <input className="search-input" placeholder="Cari nama atau email…" value={search}
                    ...
                    return (
                      <div key={u.id} className="user-card">
                        ...
                        <div className="flex gap-1.5 w-full">
                          ...
                          {allowed ? (
                            <button className="btn-danger flex-1 justify-center text-[11px] py-1.5" onClick={() => handleDeleteClick(u)}>
                              <IconTrash size={11} color="#dc2626" /> Hapus
                            </button>
                          ) : (
                            <button className="btn-danger-locked flex-1 justify-center text-[11px] py-1.5">
                              <span className="lock-tip">{lockReason}</span>
                              <IconLock size={11} /> Hapus
                            </button>
                          )}
                        </div>
                      </div>
                    ...
                    return (
                      <div key={u.id} className="list-row">
                        ...
                        <div className="flex items-center gap-1.5 flex-shrink-0">
                          ...
                          {allowed ? (
                            <button className="btn-danger" onClick={() => handleDeleteClick(u)}>
                              <IconTrash size={12} color="#dc2626" /> <span className="hidden sm:inline">Hapus</span>
                            </button>
                          ) : (
                            <button className="btn-danger-locked">
                              <span className="lock-tip">{lockReason}</span>
                              <IconLock size={12} /> <span className="hidden sm:inline">Hapus</span>
                            </button>
                          )}
                        </div>
                      </div>
